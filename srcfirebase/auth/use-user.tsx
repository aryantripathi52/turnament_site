...
import type { CoinRequest, JoinedTournament, Registration, Tournament } from '@/lib/types';
...
export const useUser = (): UserHookResult => {
  ...
  const {data: rawCoinRequests, isLoading: isCoinRequestsLoading, error: coinRequestsError } = useCollection<CoinRequest>(coinRequestsQuery);
  ...
  const joinedTournamentsQuery = useMemoFirebase(() => {
    if (!user || !firestore) return null;
    return collection(firestore, 'users', user.uid, 'joinedTournaments');
  }, [user, firestore]);
